...
export function NewRunPage({onCreated}: {onCreated: (run: RunView) => void}) {
  ...
  const [writersPerDatabase, setWritersPerDatabase] = useState(2);
  const [readersPerDatabase, setReadersPerDatabase] = useState(6);
  const [error, setError] = useState("");
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError("");
    try {onCreated(await api.createRun({
      mode, workers: mode === "performance" || mode === "fuzz" ? 1 : workers, seed,
      rounds: mode === "fuzz" ? null : rounds,
      ...
      ...(mode === "fuzz" ? {
        databases,
        writer_threads_per_database: writersPerDatabase,
        reader_threads_per_database: readersPerDatabase,
      } : {}),
    }));}
  ...
  return <main><h1>New run</h1><form onSubmit={(event) => void submit(event)}>
    <label>Mode<select value={mode} onChange={(event) => {const next = event.target.value as RunMode; setMode(next); if (next === "performance") {setWorkers(1); setQueriesPerRound(100); setTimeoutSeconds(15); setMinimumRows(100000); setMaximumRows(50000000);} else if (next === "fuzz") {setWorkers(1); setQueriesPerRound(1); setTimeoutSeconds(120); setMinimumRows(10000); setMaximumRows(10000000); setDatabases(1); setWritersPerDatabase(2); setReadersPerDatabase(6);} else {setWorkers(10); setQueriesPerRound(1000); setTimeoutSeconds(15); setMinimumRows(10); setMaximumRows(500);}}}><option value="correctness">Correctness</option><option value="performance">Performance</option><option value="fuzz">Concurrent fuzz</option></select></label>
    <label>Workers<input type="number" min="1" max="64" value={mode === "performance" || mode === "fuzz" ? 1 : workers} disabled={mode === "performance" || mode === "fuzz"} onChange={(event) => setWorkers(Number(event.target.value))}/></label>
    <label>Seed<input type="number" min="0" value={seed} onChange={(event) => setSeed(Number(event.target.value))}/></label>
    <label>Rounds<input type="number" min="1" value={rounds} disabled={mode === "fuzz"} onChange={(event) => setRounds(Number(event.target.value))}/></label>
    ...
    {mode === "fuzz" && <fieldset><legend>Fuzz concurrency</legend>
      <label>Databases<input type="number" min="1" max="32" value={databases} onChange={(event) => setDatabases(Number(event.target.value))}/></label>
      <label>Writers per database<input type="number" min="1" max="64" value={writersPerDatabase} onChange={(event) => setWritersPerDatabase(Number(event.target.value))}/></label>
      <label>Readers per database (1:2 primary/replica)<input type="number" min="3" max="192" step="3" value={readersPerDatabase} onChange={(event) => setReadersPerDatabase(Number(event.target.value))}/></label>
    </fieldset>}
