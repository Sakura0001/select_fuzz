...
- `zz_seed_fk_data.sql` 会创建 `_select_fuzz_seed_numbers` 辅助数字表，先按依赖反序清理数据，再恢复外键检查并为每张表插入 10 到 100 行可复现随机数据。
- 每张基表保留核心列后按固定 seed 随机补齐到 200 到 500 列，随机列覆盖整数、浮点、DECIMAL、日期时间、字符串、二进制、ENUM、SET、BIT 和 JSON 等类型。
...
- 每张表通过固定 seed 决定列数，生成 200 到 500 列，列类型参数和取值范围可复现随机。
- 每张表通过固定 seed 决定插入 10 到 100 行，生成结果可复现。
