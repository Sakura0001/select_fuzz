...
def _encode_artifact_value(value: object) -> object:
    ...
    if isinstance(value, Mapping):
        ...
        return {
            "$type": "mapping",
            "items": [
                [key, _encode_artifact_value(value[key])]
...
def node_execution_to_artifact(execution: NodeExecution) -> dict[str, object]:
    ...
    return {
        ...
        "rows": [
            [_encode_artifact_value(cell) for cell in row] for row in execution.rows
        ],
...
class PassRecord:
    ...
    def to_event(self) -> dict[str, object]:
        return {
            ...
            "elapsed_ns_by_role": {
                role.value: self.elapsed_ns_by_role[role] for role in NodeRole
            },
...
class FindingRecord:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "first_difference", MappingProxyType(dict(self.first_difference))
        )
        object.__setattr__(self, "statistics", MappingProxyType(dict(self.statistics)))
        fingerprints = _role_mapping(
...
class CaseBundleWriter:
    ...
    def write_finding(self, record: FindingRecord) -> Path:
        ...
        result_payloads = {
            role: _canonical_json(record.results[role]) for role in NodeRole
        }
                ...
                try:
                    ...
                    for role in NodeRole:
                        compressed = gzip.compress(
                            result_payloads[role], compresslevel=9, mtime=0
                        )
                        _write_fsynced(
